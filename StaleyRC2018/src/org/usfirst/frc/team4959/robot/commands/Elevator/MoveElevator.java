		...
		if (States.elevatorControlState == States.ElevatorControlStates.joystickControl) {
			...
			if (Robot.m_oi.getLeftStickYCont2() > Constants.JOYSTICK_Y_AXIS_DEADZONE) {
				if (!elevator.getLimitSwitch()) {
					States.elevatorPosState = States.ElevatorPosStates.userControl; // Verifies that the robot is being
			...
			// Holding position
				...
				// Dont touch unless you are willing to fight the demons that this block releases
				// if (States.elevatorSoftLimitState == States.ElevatorSoftLimitStates.enabled)
				// {
