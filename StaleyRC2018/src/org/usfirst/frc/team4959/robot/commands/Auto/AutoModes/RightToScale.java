...
/**
 * Start on the right side of the field and place the power cube in the right
 * side of the scale if we own it, otherwise cross the auto line.
 *
...
public class RightToScale extends CommandGroup {
	...
	public RightToScale() {
		...
		if (PlateColorChecker.rightScaleColor()) {
			...
			addSequential(new DriveTurn((FieldDimensions.DS_TO_SCALE - 115), 0.8, 0, 4)); // Goes straight to decision
																							// point
			addParallel(new SetElevatorPosition(Constants.ELEVATOR_HIGH_SCALE_ELEVATION)); // TODO change to High Scale
																							// for comp
			...
			// Backup sequence
			addSequential(new DriveTurn(35, -0.6, 0, 3));
			...
			// raises elevator to switch hight
			...
			// addSequential(new AutoPickupSequence()); // Pick up cube
			// addParallel(new
			// SetElevatorPosition(Constants.ELEVATOR_HIGH_SCALE_ELEVATION)); // Turn around
			// while raising elevator
...
//			addSequential(new DriveTurn(30, 0.5, 0, 1)); // Slow start to not jerk the robot
//			addParallel(new SetElevatorPosition(90000));
...
//			addSequential(new DriveTurn((60), 0.8, -0.23, 4));
//			addSequential(new DriveTurn((60), 0.5, -0.23, 4));
//			addSequential(new DriveTurn((90), 0.8, -0.23, 4));
