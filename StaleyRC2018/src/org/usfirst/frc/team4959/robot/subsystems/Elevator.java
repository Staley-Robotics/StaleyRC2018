...
public class Elevator extends Subsystem {
	...
	public Elevator() {
		motor2 = new Victor(RobotMap.ELEVATOR_MOTOR_PORT_TWO);
		// Setting up the TalonSRX
		try {
	...
	public double getTalonMotorPower() {
	...
	public void setPosition(double position) {
		...
		talon.configPeakOutputForward(LiveVariableStory.elevatorPowerLimiter, 0);
		talon.configPeakOutputReverse(-LiveVariableStory.elevatorPowerLimiter, 0);

		if ((position - getPosition()) > -4096 && (position - getPosition()) < 4096) {
			talon.configPeakOutputForward(LiveVariableStory.elevatorPowerLimiter, 0);
			talon.configPeakOutputReverse(-LiveVariableStory.elevatorPowerLimiter, 0);
		} else {
		...
		motor2.set(talon.getMotorOutputPercent());
		
		System.out.println(
				"Talon Power: " + talon.getMotorOutputPercent() + "\tSpeedController Power: " + motor2.getSpeed());

		Robot.limiter.elevatorNeedToSlowDown(RobotMap.ELEVATOR_PDP_CHANNEL);
		Robot.limiter.elevatorCanGoFaster(RobotMap.ELEVATOR_PDP_CHANNEL);
	}
	...
	public void moveElevator(double power) {
		...
		if (power <= LiveVariableStory.elevatorPowerLimiter) {
			talon.set(-power);
			motor2.set(-power);
		} else {
			talon.set(LiveVariableStory.elevatorPowerLimiter);
			motor2.set(LiveVariableStory.elevatorPowerLimiter);
		}
	}
